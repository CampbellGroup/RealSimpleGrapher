...
from RealSimpleGrapher.analysis.rabi.motional_distribution import (
    motional_distribution as md,
)
from RealSimpleGrapher.analysis.rabi.rabi_coupling import rabi_coupling as rc
...
class Rabi(Model):
    ...
    def __init__(self):
        self.parameters = {
            ...
            "sideband_order": ParameterInfo(
                "sideband_order", 4, lambda x, y: 0, vary=False
            ),
            "excitation_scaling": ParameterInfo(
                "excitation_scaling", 5, lambda x, y: 1.0, vary=False
            ),
        }
    ...
    def model(self, x, p):
        ...
        result = np.outer(p_n * effective_omega, ones) * (
                np.sin(np.outer(np.sqrt(omega ** 2 + delta ** 2) * omega_rabi / 2, x)) ** 2
        )
