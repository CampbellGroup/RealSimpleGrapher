...
class PredictSpectrum(QWidget):
    ...
    def generate_spectrum(self):
        ...
        all_carriers = self.Ca_data.get_transition_energies(
            b_field * 1e-4, line_center
        )  # to Tesla and MHz
        ...
        if self.deltam0.isChecked() == True:
            final_lines.extend(
                [
                    el
                    for el in included_lines
                    if np.abs(float(el[0][1:3]) - float(el[0][6:8])) == 0
                ]
            )
        if self.deltam1.isChecked() == True:
            final_lines.extend(
                [
                    el
                    for el in included_lines
                    if np.abs(float(el[0][1:3]) - float(el[0][6:8])) == 2
                ]
            )
        if self.deltam2.isChecked() == True:
            final_lines.extend(
                [
                    el
                    for el in included_lines
                    if np.abs(float(el[0][1:3]) - float(el[0][6:8])) == 4
                ]
            )
        ...
        for freq in carriers:
            for el in sideband_orders:
                all_lines.append(
                    ((freq + sum(np.multiply(el, sideband_freqs))), sum(np.abs(el)))
                )
...
class EnergyLevel(object):
    ...
    def magnetic_to_energy(self, B):
        ...
        energies = [
            (self.energy_scale * m * B) * 1e-6 for m in self.sublevels_m
        ]  # put in MHz
...
class Transitions_SD(object):
    ...
    def get_transition_energies(self, B, zero_offset=0.0):
    ...
    def energies_to_magnetic_field(self, transitions):
        ...
        if (
                abs(md1 - ms1) not in self.allowed_transitions
                or abs(md2 - ms2) not in self.allowed_transitions
        ):
